...
class Geometry:
    ...
    def __init__ (self):
        ...
        self.Nx = 100             # number of cells in x direction
        self.Ny = 50            # number of cells in y direction

        self.lx = 0.10             # (m) length of computational domain
        self.ly = 0.01           # (m) height of computational domain

        self.dx = self.lx / ( self.Nx )     # (m) Length of cell
        self.dy = self.ly / ( self.Ny )     # (m) Height of cell
...
class Variables:
    ...
    def __init__(self, geo):
        ...
        self.u_inlet = 0.1                    # (m/s) speed of inlet x-momentum                  
...
class Fields:        
    ...
    def __init__(self, geo):
        ...
        self.u = np.full((geo.Nx+1, geo.Ny), 0.1)           # X-momentum field
        ...
        self.Pp = np.zeros((geo.Nx-1, geo.Ny))                # Pressure correction field
...
class Faces:
    ...
    def __init__(self, geo):
        ...
        self.a_w_P = np.zeros((geo.Nx-1, geo.Ny))             # western variable coefficient of P CV for P solver
        self.a_e_P = np.zeros((geo.Nx-1, geo.Ny))             # eastern variable coefficient of P CV for P solver
        self.a_n_P = np.zeros((geo.Nx-1, geo.Ny))             # northern variable coefficient of P CV for P solver
        self.a_s_P = np.zeros((geo.Nx-1, geo.Ny))             # southern variable coefficient of P CV for P solver
        self.a_P_P = np.zeros((geo.Nx-1, geo.Ny))             # center variable coefficient of P CV for P solver

        self.b = np.zeros((geo.Nx-1, geo.Ny))
...
class TDMA:
    ...
    def __init__ (self,geo):
        ...
        self.upper_row = np.zeros((geo.Nx-1))
        self.diag_row = np.zeros((geo.Nx-1))
        self.lower_row = np.zeros((geo.Nx-1))
        self.RHS_row = np.zeros((geo.Nx-1))
        ...
        self.upper_rowv = np.zeros((geo.Nx))
        self.diag_rowv = np.zeros((geo.Nx))
        self.lower_rowv = np.zeros((geo.Nx))
        self.RHS_rowv = np.zeros((geo.Nx))

        # Pressure
        self.upper_Pcol = np.zeros((geo.Ny))
        self.diag_Pcol = np.zeros((geo.Ny))
        self.lower_Pcol = np.zeros((geo.Ny))
        self.RHS_Pcol = np.zeros((geo.Ny))

        self.upper_Prow = np.zeros((geo.Nx-1))
        self.diag_Prow = np.zeros((geo.Nx-1))
        self.lower_Prow = np.zeros((geo.Nx-1))
        self.RHS_Prow = np.zeros((geo.Nx-1))
